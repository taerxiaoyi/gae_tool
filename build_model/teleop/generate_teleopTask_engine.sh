...
# 作用：在每个子文件夹下，若存在 .onnx 文件且没有 teleopTask.engine，
#       则自动调用 trtexec 生成对应的 teleopTask.engine。
# =====================================================
...
for d in */; do
    ...
    # 查找 .onnx 文件
    onnx=$(find "$d" -maxdepth 1 -name "teleopTask.onnx" | head -n 1)
    eng="${d%/}/teleopTask.engine"
